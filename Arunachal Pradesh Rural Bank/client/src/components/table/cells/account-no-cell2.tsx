...
export default function AccountNoCell2({
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      dark:hover:bg-indigo-950/30 dark:focus-visible:ring-offset-slate-950
    "
          aria-label={`View details for account ${value ?? ''}`}
        >
          {/* Account number */}
          <span className="font-semibold text-sm text-primary dark:text-slate-100">
            {value}
          </span>
          ...
          {/* Subtle arrow */}
          <ChevronRight className="h-3.5 w-3.5 text-primary transition-all group-hover:translate-x-0.5 group-hover:text-indigo-500 dark:text-slate-500 dark:group-hover:text-indigo-300" />
        </button>
      </DialogTrigger>
