...
//         <div className='relative mt-8 h-50 w-50'>
...
//         </div>
//         {/* <h1 className='text-2xl font-extrabold text-blue-900 drop-shadow-sm dark:text-white'>
//           ARUNACHAL PRADESH RURAL BANK
//         </h1> */}
//         <h1 className='mt-4 bg-gradient-to-r from-[var(--primary)] to-[var(--foreground)] bg-clip-text text-4xl font-extrabold text-transparent'>
//           EWS-CMS
//         </h1>
...
export default function SignInPage() {
  ...
  const loginMutation = $api.useMutation('post', '/user/authenticate', {
    onSuccess: (res) => {
      ...
      setAccessToken(res.data?.token ?? '')
      if (search.redirect?.startsWith('/')) navigate({ to: search.redirect })
      else location.href = search.redirect
    },
    onError: (error) => {
  ...
  return (
    <div className='relative flex min-h-screen w-full items-center justify-center overflow-hidden bg-gradient-to-br from-[#f7fafc] via-white to-[#eef6ff] dark:from-[#0b1220] dark:via-[#0f172a] dark:to-[#0b1220]'>
      {/* Doodle layer */}
      <svg
        aria-hidden
        className='pointer-events-none absolute inset-0 h-full w-full opacity-40 dark:opacity-25'
      ...
      <div className='relative z-10 flex w-full max-w-md flex-col items-center text-center'>
        {/* Logo */}
        <div className='mb-2 h-40 w-auto'>
          <img
            src={ViteLogo}
            alt='SSBD Logo'
        ...
        {/* Title */}
        <h1 className='bg-gradient-to-r from-[var(--primary)] to-[var(--foreground)] bg-clip-text text-4xl font-extrabold text-transparent'>
          EWS-CMS
        </h1>
        ...
        {/* Form */}
        <div className='mt-6 w-full rounded-3xl border border-slate-200/70 bg-white/90 p-6 shadow-[0_20px_60px_-20px_rgba(0,0,0,0.35)] backdrop-blur-md dark:border-slate-700/60 dark:bg-slate-900/85'>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-6'>
              <FormField
                control={form.control}
              ...
              <Button
                type='submit'
                className='relative mt-2 h-11 w-full rounded-xl text-[15px] font-bold text-white shadow-lg focus-visible:ring-2 focus-visible:ring-sky-400 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-70'
                disabled={isLoading}
              >
                <span className='relative z-10'>
                  {isLoading ? 'Logging in…' : 'Login'}
