...
export default function LoanReview() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    const url = editing
      ? `${backendUrl}/loan-review/update/${editing}`
      : `${backendUrl}/loan-review/create`
    try {
      ...
      if (!editing) setLoanReviewHistory((prev) => [...prev, result?.data])
      reset(defaultValues)
      setEditing(null)
    } catch (error) {
  ...
  const getLeadData = async (data: LeadFormData) => {
    try {
      const token = sessionStorage.getItem('token')
      const url = `${backendUrl}/loan-review/fetchData?acctNo=${data.accountNo}&lead_no=${data.leadNo}&type=${data.type}`
      const response = await fetch(url, {
        method: 'GET',
        headers: { Authorization: `Bearer ${token}` },
      })
      if (!response.ok) throw new Error('Failed to fetch lead data')
      const result: CreditAppraisalResponse = await response.json()
      setFormVisible(true)
      setYearsList(result?.financial_details?.acc_year_end || [])
      setFinancialDataFetched(result?.financial_details)
      reset({
        ...defaultValues,
        constitution: result?.customerDetails?.constitution || '',
        borrowerName: result?.customerDetails?.acctNm || '',
        personalGuarantees: result?.guarantor_details || [{ name: '' }],
        natureOfBusiness: result?.nature_of_business || '',
        limitFacilitiesSince: result?.customerDetails?.opDate || '',
        creditFacilities:
          result?.creditFacilities || defaultValues.creditFacilities,
        sisterConcernCreditFacilities:
          result?.sisterConcernCreditFacilities ||
          defaultValues.sisterConcernCreditFacilities,
        collateralSecurities:
          result?.collateralSecurities || defaultValues.collateralSecurities,
      })
  ...
  const getLoanHistory = async () => {
    try {
      const url = `${backendUrl}/loan-review/get?acctNo=${accountId}`
      const token = sessionStorage.getItem('token')
      const response = await fetch(url, {
        method: 'GET',
        headers: { Authorization: `Bearer ${token}` },
      ...
      const data: LoanReviewHistory[] = await response.json()
      setLoanReviewHistory(data || [])
    } catch (error) {
      toast.error((error as Error).message || 'An error occurred')
    }
  }
  ...
  const deleteRecord = async (id: string) => {
    ...
    try {
      ...
      const result = await response.json()
      setLoanReviewHistory(loanReviewHistory.filter((item) => item.id !== id))
