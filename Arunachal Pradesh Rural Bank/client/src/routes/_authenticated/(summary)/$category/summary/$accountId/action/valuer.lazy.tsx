...
function RouteComponent() {
  ...
  const updateEventMutation = $api.useMutation('put', '/valuer/events/{id}/update', {
    onSuccess: () => {
  ...
  return (
    <>
      ...
      <Main className='px-4 py-2'>
        ...
        <SecurityDetailsSection accountNo={accountNo} title='Security Details' />
            ...
            <div className='relative border-b bg-gradient-to-br from-background via-muted/30 to-background px-6 py-5'>
              ...
              <DialogHeader className='relative space-y-1'>
                ...
                <p className='text-sm text-muted-foreground'>
                  Assign a valuer and link the event to a security with a due date.
                </p>
              </DialogHeader>
            </div>
                  ...
                  <div className='rounded-xl border bg-card p-4 shadow-sm'>
                    ...
                    >
                      <SelectTrigger className='mt-2 w-full'>
                        <SelectValue
                          placeholder={isValuerLoading ? 'Loading valuers…' : 'Select Valuer'}
                        />
                      </SelectTrigger>

                      <SelectContent className='max-h-64'>
                        {valuers.map((v) => (
                          <SelectItem key={String(v.id ?? '')} value={normalize(v.id)}>
