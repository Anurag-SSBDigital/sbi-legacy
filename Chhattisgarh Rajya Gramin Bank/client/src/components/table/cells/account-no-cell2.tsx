...
// ---------------- Component ----------------
export default function AccountNoCell2({
  value,
}: {
  value: string | undefined
}) {
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogTrigger asChild>
        <button
          className='group inline-flex items-center gap-2 rounded-lg px-2 py-1 text-left transition-all duration-200 hover:bg-indigo-50/60 focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2 focus-visible:outline-none dark:hover:bg-indigo-950/30 dark:focus-visible:ring-offset-slate-950'
          aria-label={`View details for account ${value ?? ''}`}
        >
          {/* Account number */}
          <span className='text-primary text-sm font-semibold dark:text-slate-100'>
            {value}
          </span>
          ...
          {/* Subtle arrow */}
          <ChevronRight className='text-primary h-3.5 w-3.5 transition-all group-hover:translate-x-0.5 group-hover:text-indigo-500 dark:text-slate-500 dark:group-hover:text-indigo-300' />
        </button>
      </DialogTrigger>
