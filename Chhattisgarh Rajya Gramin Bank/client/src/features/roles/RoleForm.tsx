...
const pickModule = (
  ...
  key: string
): PermissionSection => (section[key] ? { [key]: section[key] } : {})
...
export default function RoleForm({
  ...
  const { data: permissionSections = {}, isLoading: permissionCatalogLoading } =
    usePermissionSections()
  const flatPermissions = useMemo(
    () => getFlatPermissions(permissionSections),
    [permissionSections]
  )
  ...
  useEffect(() => {
    if (!Object.keys(flatPermissions).length) return
    const current = { ...(getValues('permissions') ?? {}) }
    let changed = false

    Object.keys(current).forEach((module) => {
      const allowedActionsForModule = flatPermissions[module]
      if (!Array.isArray(allowedActionsForModule)) {
        delete current[module]
        changed = true
        return
      }

      const existingActions = Array.isArray(current[module])
        ? current[module]
        : []
      const allowedActions = new Set(allowedActionsForModule)
      const filtered = existingActions.filter((action) =>
        allowedActions.has(action)
      )
      if (filtered.length !== existingActions.length) {
        current[module] = filtered
        changed = true
      }
    })

    Object.keys(flatPermissions).forEach((module) => {
      if (!Array.isArray(current[module])) {
        current[module] = []
        changed = true
      }
    })
    ...
    if (changed) {
      setValue('permissions', current, { shouldDirty: false })
    }
  }, [flatPermissions, getValues, setValue])
  ...
  const filteredPermissionSections = useMemo<PermissionSections>(() => {
    if (!roleLevel) return permissionSections

    /* split “Masters” from the rest */
    const { Masters = {}, Alerts = {}, ...others } = permissionSections
    ...
    if (roleLevel === 'Department') {
      mastersSubset = {
        ...pickModule(Masters, 'departments'),
        ...pickModule(Masters, 'auditors'),
        ...pickModule(Masters, 'advocates'),
        ...pickModule(Masters, 'roles'),
        ...pickModule(Masters, 'users'),
        ...pickModule(Masters, 'process_settings'),
        ...pickModule(Masters, 'data-ingestion'),
      }
      alertsSubset = { ...Alerts }
      if (Alerts.ews_alert?.length) {
        alertsSubset.ews_alert = [Alerts.ews_alert[0]]
      }
    }
    if (roleLevel === 'Branch') {
      mastersSubset = {
    ...
    return Object.fromEntries(
      Object.entries({
        Masters: mastersSubset,
        ...others,
        Alerts: alertsSubset,
      }).filter(([, modules]) => Object.keys(modules).length > 0)
    ) as PermissionSections
  }, [roleLevel, permissionSections])
          ...
          <section className='space-y-8'>
            ...
            <div className='space-y-6'>
              {permissionCatalogLoading &&
                Object.keys(filteredPermissionSections).length === 0 && (
              ...
              {!permissionCatalogLoading &&
                Object.keys(filteredPermissionSections).length === 0 && (
                  <div className='text-muted-foreground rounded-md border p-4 text-sm'>
                    No permissions available. Confirm access to{' '}
                    <code>/permissions/catalog</code>.
                  </div>
                )}
