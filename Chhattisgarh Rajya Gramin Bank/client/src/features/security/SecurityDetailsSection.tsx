...
export default function SecurityDetailsSection({
  ...
  const transformedData: SecurityRow[] = useMemo(
    () => toSecurityRows(data),
    [data]
  )
  ...
  const securityTypeOptions = useMemo<string[]>(() => {
    return extractArray(securityTypeResp)
      .map((x) =>
        isRecord(x) && typeof x.securityType === 'string'
          ? x.securityType
          : null
      )
  ...
  const updateSecurityMutation = $api.useMutation(
    'put',
    '/legal/security-details/update/{id}',
    {
      onSuccess: () => {
    ...
    form.reset(
      {
        ...
        realizableValue: Number.isFinite(row.realizableValue)
          ? row.realizableValue
          : 0,
    ...
    <>
      <div className='mb-6 flex items-center justify-between'>
        ...
        >
          ...
          {/* ✅ Scrollable dialog */}
          <DialogContent className='max-h-[90vh] w-[calc(100vw-1.5rem)] overflow-hidden p-0 sm:max-w-xl md:max-w-2xl lg:max-w-3xl'>
            <div className='flex max-h-[90vh] flex-col'>
              {/* Header */}
              <div className='from-background via-muted/30 to-background relative border-b bg-gradient-to-br px-6 py-4'>
              ...
              <Form {...form}>
                ...
                >
                          ...
                          render={({ field }) => (
                            <FormItem>
                              ...
                              <FormControl>
                                <Input
                                  {...field}
                                  placeholder='e.g. Land + building details'
                                  required
                                />
                              </FormControl>
                              <FormMessage />
                                ...
                                <div className='space-y-3'>
                                  <Select
                                    onValueChange={(v) => {
                                      field.onChange(v)
                                      if (v !== 'Other')
                                        form.setValue('customSecurityType', '')
                                    }}
                                  ...
                                  <div className='bg-muted/20 text-muted-foreground rounded-lg border px-3 py-2 text-xs'>
                                    Tip: Choose “Other” only if the type is not
                  ...
                  {/* Footer */}
                  <div className='bg-background/80 shrink-0 border-t px-6 py-4 backdrop-blur'>
                    <DialogFooter className='flex flex-col gap-2 sm:flex-row sm:justify-end'>
                      ...
                      <Button
                        type='submit'
                        className='shadow-sm sm:min-w-[160px]'
                        disabled={
          ...
          { key: 'realizableValue', label: 'Realizable Value' },
          {
            ...
            render: (_, row) => (
              <Button
                size='sm'
                variant='outline'
                onClick={() => handleEditSecurity(row as SecurityRow)}
              >
                Edit
              </Button>
            ),
