...
import {
  profilePictureQueryKeys,
  useProfilePicture,
} from '@/hooks/use-profile-picture.ts'
...
const profileFormSchema = z.object({
  ...
  fullname: z
    ...
    .regex(
      /^[\p{L} .'-]+$/u,
      'Name can include letters, spaces, apostrophes, periods, and hyphens only'
    ),
})
  ...
  const uploadPicMutation = $api.useMutation('post', '/user/uploadProfilePic', {
    onSuccess: (res) => {
      ...
      const uploadedFilename =
        typeof res.data === 'string' && res.data.trim().length > 0
          ? res.data.trim()
          : undefined
      if (user) {
        setUser({
          ...user,
          profilePic: uploadedFilename ?? user.profilePic,
        })
      }
      if (user?.username) {
        queryClient.removeQueries({
          queryKey: profilePictureQueryKeys.byUser(user.username),
          exact: false,
        })
      }
