...
import {
  ...
  inList,
  isRecord,
} from '@/features/security/security-helpers'
...
function RouteComponent() {
  ...
  const eventTypeOptions = useMemo<string[]>(() => {
    return extractArray(eventTypeResp)
      .map((x) =>
        isRecord(x) && typeof x.eventType === 'string'
          ? String(x.eventType)
          : null
      )
  ...
  const { data: advocateDropdownData, isLoading: isAdvocateLoading } =
    $api.useQuery('get', '/advocate/dropdown', {})
  ...
  const transformedEventData: EventRow[] = Array.isArray(
    ...
    ? ((eventListData as { data?: unknown[] }).data ?? []).map(
        (item: unknown) => {
          if (!isRecord(item)) {
            return {
              id: undefined,
          ...
          const assignment = isRecord(item.assignment)
            ? item.assignment
            : undefined
          const advocateDetails = isRecord(item.advocateDetails)
            ? item.advocateDetails
            : undefined
  ...
  /* ---------------- Mutations ---------------- */
  const assignLegalEventMutation = $api.useMutation(
    'post',
    '/legal/events/create-and-assign',
    {
      onSuccess: () => {
  ...
  const handleDownload = async (eventId: number | string) => {
    const idNum =
      typeof eventId === 'number' ? eventId : Number.parseInt(eventId, 10)
            ...
            <div className='from-background via-muted/30 to-background relative border-b bg-gradient-to-br px-6 py-5'>
              ...
              <DialogHeader className='relative space-y-1'>
                ...
                <p className='text-muted-foreground text-sm'>
                  Choose event type, assign advocate, link security, and set due
                  date.
                </p>
              </DialogHeader>
            </div>
                ...
                <div className='bg-card rounded-xl border p-4 shadow-sm'>
                  ...
                  >
                    <SelectTrigger className='mt-2 w-full'>
                      <SelectValue
                        placeholder={
                          isEventTypeLoading ? 'Loading…' : 'Select Event Type'
                        }
                      />
                    </SelectTrigger>
