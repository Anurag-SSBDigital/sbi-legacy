    ...
    <MainWrapper>
      ...
      >
        <header className='space-y-4 text-center'>
          <div className='border-border/80 bg-muted/40 mt-4 inline-flex items-center justify-center gap-2 rounded-full border px-4 py-2 text-[24px] font-bold uppercase'>
            <span className='bg-primary/80 inline-flex h-3 w-3 rounded-full' />
            Account Summary
          </div>

          <div className='via-border/90 mt-4 h-1 w-full bg-gradient-to-r from-transparent to-transparent' />
        </header>

        <section className='border-border/70 bg-card/80 relative mt-4 rounded-2xl border p-5 shadow-sm'>
          <span className='bg-primary/70 absolute inset-y-4 left-0 w-1 rounded-full' />

          <div className='border-border/60 flex items-center justify-between border-b pb-4 pl-3'>
            ...
            <div className='grid gap-4 sm:grid-cols-2 lg:grid-cols-4'>
              {acctLoading ? (
                ...
                <>
                  ...
                  <StatsCard
                    label='Account No'
                    value={<AccountNoCell2 value={account?.acctNo ?? '—'} />}
                  />
                  <StatsCard
                    label='Outstanding'
        ...
        <section className='border-border/70 bg-card/80 relative mt-6 rounded-2xl border p-5 shadow-sm'>
          <span className='bg-primary/70 absolute inset-y-4 left-0 w-1 rounded-full' />
          <div className='border-border/60 flex items-center justify-between border-b pb-4 pl-3'>
            <div className='space-y-1'>
              <h2 className='text-muted-foreground text-sm font-semibold tracking-[0.18em] uppercase'>
...
const Detail = ({ label, value }: DetailProps) => (
  <div className='border-muted rounded-lg border p-4'>
    ...
    <p className='mt-4 text-lg font-semibold break-all'>{value ?? '—'}</p>
  </div>
)
...
function ActionGrid({
  ...
  return (
    <div className='grid gap-3 sm:grid-cols-2 lg:grid-cols-3'>
      ...
      {canGenerateReport && (
        <div className='border-border/60 from-muted/60 via-card/90 relative col-span-full mt-6 overflow-hidden rounded-2xl border bg-gradient-to-tr to-transparent p-6 shadow-sm transition-all duration-300'>
          <div className='flex flex-col items-center gap-4 text-center'>
            <div className='bg-primary/10 text-primary inline-flex items-center gap-2 rounded-full px-4 py-1 text-xs font-medium'>
            ...
            <p className='text-muted-foreground max-w-xl text-sm leading-relaxed'>
              Includes inspection notes, interviews, legal actions, notices and
              follow-ups — compiled in a single structured PDF.
            </p>

            <a
              href={`${BASE_URL}/AlertReport/generate/${accountId}`}
              download
              className='group border-primary/40 bg-primary/90 text-primary-foreground hover:bg-primary inline-flex items-center gap-2 rounded-lg border px-6 py-2.5 font-medium shadow transition-all duration-200'
            >
