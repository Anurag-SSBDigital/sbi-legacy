...
function RouteComponent() {
  ...
  const canCreateTransfer = useCanAccess('auca', 'create_transfer')
  const canInitiate = useCanAccess('auca', 'initiate')
  const canDownloadDocument = useCanAccess('auca', 'download_document')
  ...
  const downloadAnnexureAMutation = useMutationShallow(
    'get',
    '/auca-transfer/tgb/pdf/annexure-a/{accountNumber}'
  )
  ...
  const handleDownloadSummaryReport = async () => {
    if (!canDownloadDocument) {
      toast.error('You do not have permission to download AUCA documents.')
      return
    }
  ...
  const handleDownloadAnnexureA = async (accountNumber: string) => {
    ...
    try {
      ...
      if (!blob) {
        toast.error('Failed to download Annexure A.')
        return
      }

      const filename = `Annexure-A-${accountNumber}.pdf`
      ...
      toast.success('Annexure A download started.')
    } catch (err) {
      toastError(err, 'Failed to download Annexure A.')
    }
  }
  ...
  const initiateMutation = (accountNumber: string) =>
    canInitiate
      ...
      : toast.error('You do not have permission to initiate AUCA.')
                ...
                renderActions={(row) => {
                  ...
                  return (
                    <div className='flex flex-row justify-end gap-1'>
                      ...
                      {!row.transferInitiated && canCreateTransfer && (
                        <Link
                          to='/auca/transfer'
                      ...
                      {canDownloadDocument && row.transferInitiated && (
                        <Button
                          type='button'
                          variant='outline'
                          className={baseButtonClass}
              ...
              <div className='flex flex-col items-center gap-2'>
                ...
                {selectedAccountNumber && (
                  <Button
                    ...
                    disabled={!canDownloadDocument || downloadingSummary}
                  >
                    <DownloadIcon />
