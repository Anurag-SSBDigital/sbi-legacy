...
export const Route = createFileRoute('/_authenticated/ots/eligible')({
...
/* -----------------------------
   OTS Stage-wise summary response
------------------------------ */
type OtsDocument = {
...
type OtsStageWiseSummaryResponse = {
...
const fmtDT = (s?: string | null) => {
  if (!s) return '-'
  // keep it simple; your timestamps are ISO
...
function RouteComponent() {
  ...
  // ✅ per-row download loader
  const [downloadingId, setDownloadingId] = useState<number | null>(null)

  const user = useAuthStore().auth.user
  const isBranchDropdownVisible = user?.superAdmin || user?.admin || false
  const canInitiate = useCanAccess('ots', 'initiate')
  const canCreateProposal = useCanAccess('ots', 'create_proposal')
  const canDownloadDocument = useCanAccess('ots', 'download_document')
  ...
  const {
    ...
    error,
    refetch: refetchList,
  } = $api.useQuery('get', '/otsproposal/OtsAccountList')

  const initiateOtsMutation = $api.useMutation(
    'post',
    '/api/ots/{accountNumber}/initiate'
  )

  // ✅ Combined Summary PDF download mutation
  const downloadCombinedReportMutation = $api.useMutation(
    'get',
    '/api/ots/{accountNumber}/combined-report.pdf'
  )

  const handleDownloadCombinedReport = (acctNo?: string | null) => {
    if (!canDownloadDocument) {
      toast.error('You do not have permission to download OTS documents.')
      return
    }

    const accountNumber = String(acctNo ?? selectedAccountNumber ?? '')
    if (!accountNumber) {
      toast.error('Account number not available.')
      return
    }

    downloadCombinedReportMutation.mutate(
      {
        params: {
          path: { accountNumber },
        },
      ...
      },
      {
        onSuccess: (res: unknown) => {
          const payload: unknown = hasData<unknown>(res) ? res.data : res

          let blob: Blob | null = null
          if (isBlob(payload)) blob = payload
          else if (isArrayBuffer(payload))
            blob = new Blob([payload], { type: 'application/pdf' })
          else if (isUint8Array(payload)) {
            const buf = payload.buffer as ArrayBuffer
            const ab = buf.slice(
              payload.byteOffset,
          ...
          a.href = url
          a.download = `OTS_Summary_Report_${accountNumber}.pdf`
        ...
        onError: () => {
          // fallback open (keeps behavior consistent with your other download)
          window.open(
            `${BASE_URL}/api/ots/${encodeURIComponent(accountNumber)}/combined-report.pdf`,
            '_blank',
            'noopener,noreferrer'
          )
        },
  ...
  const initiateMutation = (accountNumber: string) =>
    canInitiate
      ? initiateOtsMutation.mutate(
          {
            params: { header: { Authorization: '' }, path: { accountNumber } },
          },
          { onSuccess: () => refetchList() }
        )
      : toast.error('You do not have permission to initiate OTS.')
  ...
  // ✅ PDF download mutation (same as your current code)
  const downloadPdfMutation = $api.useMutation(
    'get',
    '/otsproposal/tgb/proposals/by-account/{accountNo}/pdf'
  )

  const handleDownloadPdf = (id: number, acctNo?: string | null) => {
    if (!canDownloadDocument) {
      toast.error('You do not have permission to download OTS documents.')
      return
    }
    ...
    downloadPdfMutation.mutate(
      {
        params: {
          path: { accountNo: String(acctNo) },
          query: { type: 'ANNEXURE_I ' },
        },
        parseAs: 'blob',
      },
      {
        onSuccess: (res: unknown) => {
          ...
          if (!blob) {
            toast.error('Could not download Proposal. Please try again.')
            return
          }
          ...
          const a = document.createElement('a')
          a.href = url
          a.download = `OTS_Proposal_${acctNo ?? id}.pdf`
          document.body.appendChild(a)
          a.click()
          a.remove()
          URL.revokeObjectURL(url)
        ...
        onError: () => {
          window.open(`/otsproposal/${id}/pdf`, '_blank', 'noopener,noreferrer')
        },
  ...
  const stageSummary: OtsStageWiseSummaryResponse | null = useMemo(() => {
    if (!stageWiseSummary) return null
    // if backend wraps in {data: ...} later, this still works
    if (hasData<OtsStageWiseSummaryResponse>(stageWiseSummary))
      return stageWiseSummary.data
    return stageWiseSummary as unknown as OtsStageWiseSummaryResponse
  }, [stageWiseSummary])

  const stages: OtsStage[] = stageSummary?.stages ?? []
  const history: OtsHistoryItem[] = stageSummary?.history ?? []
  ...
  const accounts = useMemo(() => listResponse?.data ?? [], [listResponse])
    ...
    <>
      ...
      <Main className='px-4 py-2'>
        <AppBreadcrumb
          className='p-2'
          crumbs={[Home]}
          currentPage={{ type: 'label', label: 'OTS Eligible Accounts' }}
        />
        ...
        {isLoading ? (
          ...
          <div className='py-10 text-center text-red-500'>
            Error fetching eligible OTS accounts:{' '}
            {String((error as Error).message)}
          </div>
        ) : !accounts || accounts.length === 0 ? (
          <div className='py-10 text-center text-gray-500'>
            No OTS eligible accounts available.
          </div>
        ) : (
          <Card className='col-span-full shadow-lg'>
            <CardHeader className='flex flex-col gap-4 sm:flex-row sm:items-baseline sm:justify-between'>
              <div>
                <CardTitle className='text-2xl font-bold text-gray-800 dark:text-gray-200'>
                  OTS Eligible Accounts ({filteredData.length} accounts)
                </CardTitle>
              ...
              <div className='flex flex-row items-center gap-2'>
                ...
                <Input
                  ...
                  aria-label='Search eligible OTS accounts'
                />
              </div>
            </CardHeader>
            ...
            <CardContent className='px-6'>
              <PaginatedTable
                ...
                data={filteredData}
                columns={columns}
                emptyMessage='No OTS eligible accounts to show.'
                renderActions={(row) => {
                  ...
                  const completed =
                    row.praposalCreated === true && row.initiated === true
                  ...
                  return (
                    <div className='flex flex-row justify-end gap-2'>
                      ...
                      {!row.praposalCreated && canCreateProposal && (
                        <Link
                          to='/ots/initiate-ots-compromise'
                      ...
                      {canInitiate && row.praposalCreated && !row.initiated && (
                        <Button
                          type='button'
                          variant='outline'
                          onClick={() =>
                      ...
                      {canDownloadDocument && row.praposalCreated && (
                        <Button
                          type='button'
                          variant='outline'
                          className={baseButtonClass}
      ...
      {/* Confirm Initiate OTS */}
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Are you sure?</AlertDialogTitle>
            <AlertDialogDescription>
              This action will initiate the OTS process for account{' '}
            ...
            <div className='flex items-start justify-between gap-4'>
              <div className='min-w-0 flex-1 space-y-1'>
                ...
                <DialogDescription className='text-xs sm:text-sm'>
                  Combined view of core account information, OTS process status,
                  and stage-wise history for quick review.
                </DialogDescription>
              </div>

              {(stageSummary?.instanceStatus || selectedAccountNumber) && (
                <div className='flex flex-col items-end gap-2'>
                  ...
                  <Button
                    ...
                    variant='outline'
                    size='sm'
                    className='h-8 gap-2 text-[0.72rem]'
                    disabled={
                      !canDownloadDocument ||
                      downloadCombinedReportMutation.isPending
                    }
                    onClick={() =>
                ...
                <div className='grid gap-4 md:grid-cols-3'>
                  ...
                  <div className='from-muted/40 to-background rounded-xl border bg-gradient-to-b p-4 shadow-sm'>
                    ...
                    {statusData ? (
                      ...
                      <p className='text-muted-foreground text-xs'>
                        OTS process not yet initiated for this account.
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div className='space-y-3'>
                  <div>
                    ...
                    <p className='text-muted-foreground text-[0.75rem]'>
                      Timeline of each OTS stage with visits, tasks, actions &
                      documents.
                    </p>
                  </div>
                  ...
                  {Array.isArray(stages) && stages.length > 0 ? (
                    ...
                    <p className='text-muted-foreground text-xs'>
                      No stage history available yet. Once OTS is initiated and
                      stages progress, you’ll see a timeline here.
                    </p>
                  )}
                </div>
