...
const columns: PaginatedTableProps<OtsAccountRow>['columns'] = [
  {
    key: 'acctNo',
    label: 'Account No',
    render: (value) =>
...
export const Route = createFileRoute('/_authenticated/ots/')({
  component: RouteComponent,
})

function RouteComponent() {
  const canMarkEligible = useCanAccess('ots', 'mark_eligible')
  ...
  const {
    ...
    error,
    refetch,
  } = $api.useQuery('get', '/otsproposal/accountlist', {
    params: {
      header: {
  ...
  const accounts: OtsAccountRow[] = useMemo(() => {
    if (!listResponse?.data) return []
    const data = listResponse.data as {
      content?: OtsAccountRow[]
      currentPage?: number
      pageSize?: number
  ...
  const handleMarkEligible = () => {
    if (!canMarkEligible) {
      toast.error('You do not have permission to mark OTS eligibility.')
      return
    }
    ...
    markEligible(
      ...
      },
      {
        onSuccess: () => {
          ...
          toast.success('Account marked as OTS eligible')
        ...
        onError: () => {
          setIsMarkingEligible(false)
          toast.error('Failed to mark account as OTS eligible')
        },
      }
    )
  }
      ...
      <Main className='px-4 py-2'>
        ...
        {isLoading ? (
          ...
          <div className='py-10 text-center text-gray-500'>
            No OTS accounts available for this branch.
          </div>
        ) : (
          <Card className='col-span-full shadow-lg'>
            <CardHeader className='flex flex-col gap-4 sm:flex-row sm:items-baseline sm:justify-between'>
              <div>
                <CardTitle className='text-2xl font-bold text-gray-800 dark:text-gray-200'>
                  OTS Accounts ({filteredData.length} accounts)
                </CardTitle>
              ...
              <div className='flex flex-row items-center gap-2'>
                ...
                <Input
                  ...
                  aria-label='Search OTS accounts'
                />
              </div>
            </CardHeader>
            ...
            <CardContent className='px-6'>
              <PaginatedTable
                ...
                data={filteredData}
                columns={columns}
                emptyMessage='No OTS accounts to show.'
                renderActions={(row) =>
                  row.acctNo ? (
                    <Button
                      variant='outline'
        ...
        >
          <DialogContent className='max-h-[85vh] overflow-y-auto sm:max-w-4xl'>
            ...
            <DialogFooter className='mt-6 flex flex-col gap-2 border-t pt-4 sm:flex-row sm:justify-between'>
              ...
              >
                {isMarkingEligible ? 'Marking…' : 'Mark as OTS Eligible'}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
