...
function RouteComponent() {
  ...
  const user = useAuthStore().auth.user
  const isBranchDropdownVisible = user?.superAdmin || user?.admin || false
  const canCreateReview = useCanAccess('problem_loan_review', 'create_review')
  const canInitiate = useCanAccess('problem_loan_review', 'initiate')
  const canDownloadDocument = useCanAccess(
    'problem_loan_review',
    'download_document'
  )
  ...
  const stageSummary: PlrStageWiseSummaryResponse | null = useMemo(() => {
    ...
    if (hasData<PlrStageWiseSummaryResponse>(stageWiseSummary))
                ...
                renderActions={(row) => {
                  ...
                  return (
                    <div className='flex flex-row justify-end gap-1'>
                      ...
                      {!row.reviewCreated && canCreateReview && (
                        <Link
                          to='/problem-loan-review/review/$accountNumber'
                          params={{ accountNumber: row.accountNumber }}
                        >
      ...
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleConfirm} disabled={!canInitiate}>
              Confirm
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
            ...
            <div className='flex items-start justify-between gap-4'>
              <div className='min-w-0 flex-1 space-y-1'>
                ...
                <DialogDescription className='text-xs sm:text-sm'>
                  Combined view of core account information, process status, and
                  stage-wise history for quick review.
                </DialogDescription>
              </div>
              ...
              <div className='space-y-6'>
                ...
                <div className='grid gap-4 md:grid-cols-3'>
                      ...
                      <div className='grid gap-3 text-xs sm:grid-cols-2'>
                        ...
                        <div className='bg-background/60 rounded-lg border p-3'>
                          ...
                          <p className='mt-1 text-sm font-semibold'>
                            {(
                              accountDetails.outstandingAmount ?? 0
                  ...
                  <div className='from-muted/40 to-background rounded-xl border bg-gradient-to-b p-4 shadow-sm'>
                    ...
                    {statusData ? (
                      ...
                      <p className='text-muted-foreground text-xs'>
                        Process not yet initiated for this account.
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div className='space-y-3'>
                  ...
                  {Array.isArray(stages) && stages.length > 0 ? (
                    ...
                    <p className='text-muted-foreground text-xs'>
                      No stage history available yet. Once initiated and stages
                      progress, you’ll see a timeline here.
                    </p>
                  )}
                </div>
