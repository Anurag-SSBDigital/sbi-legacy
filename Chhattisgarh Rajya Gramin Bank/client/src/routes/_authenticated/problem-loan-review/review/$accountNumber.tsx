...
function RouteComponent() {
    ...
    fetchAccountDetails(
      ...
      },
      {
        onSuccess: (res) => {
          ...
          if (!customer) {
            toast.error(
              body.message || 'Could not load account details for prefill'
            )
            return
          }
          ...
          form.reset({
            ...
            sanctionedLimit:
              customer.loanLimit !== undefined
                ? String(customer.loanLimit)
                : '',
          ...
          setIfBlankAndNotDirty(
            'lastSanctionDate',
            normalizeDateOnly(customer.sanctDt)
          )
          setIfBlankAndNotDirty(
            'sanctionedLimit',
  ...
  const onSubmit = (values: FormValues) => {
    if (!canCreateReview) {
      toast.error('You do not have permission to create problem loan reviews.')
      return
    }
  ...
  const isSaving = createProblemLoanReviewMutation.isPending
  const canSubmit = canCreateReview
