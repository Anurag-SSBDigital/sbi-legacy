...
type RecalledAssetsEligibleAccount =
  components['schemas']['RecalledAssetsEligibleAccount']

export const Route = createFileRoute('/_authenticated/recalled-assets/')({
  component: RouteComponent,
})

function RouteComponent() {
  const canMarkEligible = useCanAccess('recalled_assets', 'mark_eligible')

  const [branchId, setBranchId] = useState<string | undefined>(undefined)
  const [query, setQuery] = useState('')
  const [detailsDialogOpen, setDetailsDialogOpen] = useState(false)
  const [selectedAccount, setSelectedAccount] =
    useState<RecalledAssetsEligibleAccount | null>(null)
  ...
  const {
    ...
    error,
    refetch,
  } = $api.useQuery('get', '/RecalledAssets/accountlist', {
    params: {
  ...
  const { mutate: markEligible } = $api.useMutation(
    'post',
    '/RecalledAssets/EligibleAccount'
  )

  const handleViewDetails = (accountNumber?: string) => {
  ...
  const handleMarkEligible = () => {
    if (!canMarkEligible) {
      toast.error(
        'You do not have permission to mark recalled assets eligibility.'
      )
      return
    }
    ...
    markEligible(
      ...
      },
      {
        onSuccess: () => {
          ...
          toast.success('Account marked as Recalled Asset eligible')
        ...
        onError: () => {
          setIsMarkingEligible(false)
          toast.error('Failed to mark account as Recalled Asset eligible')
        },
      }
    )
  }
      ...
      <Main className='px-4 py-2'>
        ...
        {isLoading ? (
          ...
          <div className='py-10 text-center text-gray-500'>
            No Recalled Asset accounts available for this branch.
          </div>
        ) : (
          <Card className='col-span-full shadow-lg'>
            <CardHeader className='flex flex-col gap-4 sm:flex-row sm:items-baseline sm:justify-between'>
              <div>
                <CardTitle className='text-2xl font-bold text-gray-800 dark:text-gray-200'>
                  Recalled Asset Accounts ({filteredData.length} accounts)
                </CardTitle>
              ...
              <div className='flex flex-row items-center gap-2'>
                <Label htmlFor='searchAccounts' className='sr-only'>
                  Search Recalled Asset accounts
                </Label>
                <Input
                  id='searchAccounts'
                  ...
                  aria-label='Search Recalled Asset accounts'
                />
              </div>
            </CardHeader>
            ...
            <CardContent className='px-6'>
              <PaginatedTable
                ...
                data={filteredData}
                columns={columns}
                emptyMessage='No Recalled Asset accounts to show.'
                renderActions={(row) =>
                  row.acctNo ? (
                    <Button
                      variant='outline'
          ...
          <DialogContent className='max-h-[85vh] overflow-y-auto sm:max-w-4xl'>
            <DialogHeader className='space-y-1'>
              <DialogTitle className='flex items-center justify-between gap-3 text-lg font-semibold'>
                <span>Recalled Asset Account Details</span>
              ...
              <div className='mt-4 space-y-6 text-sm'>
                ...
                <div className='bg-muted/40 grid gap-3 rounded-xl border p-4 sm:grid-cols-3'>
                  ...
                  <div className='flex flex-col gap-1'>
                    ...
                    <span className='font-mono text-base font-semibold'>
                      {selectedAccount.loanSanctionAmount !== undefined
                        ? selectedAccount?.loanSanctionAmount?.toLocaleString(
                            'en-IN',
                            {
                              maximumFractionDigits: 2,
                            }
                  ...
                  <h3 className='text-muted-foreground flex items-center justify-between text-xs font-semibold tracking-wide uppercase'>
                    ...
                    <span
                      className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[11px] font-medium ${
                        selectedAccount.recalledEligible
                ...
                <section className='bg-background/60 space-y-3 rounded-xl border p-4 shadow-sm'>
                  ...
                  <div className='bg-muted/40 grid gap-2 rounded-lg p-3 sm:grid-cols-2'>
                    ...
                    <DetailRow
                      ...
                      value={selectedAccount.loanSanctionAmount}
                    />
                    <DetailRow
                      label='NPA Date'
                ...
                <section className='bg-background/60 space-y-3 rounded-xl border p-4 shadow-sm'>
                  ...
                  <div className='bg-muted/40 grid gap-2 rounded-lg p-3 sm:grid-cols-2'>
                    <DetailRow
                      ...
                      value={selectedAccount.securityValue}
                    />
                    <DetailRow
                      label='Security Details'
                      value={selectedAccount.securityDetails}
                    />
                    <DetailRow
                      label='Guarantor Details'
                      value={selectedAccount.guarantorDetails}
                    />
                    <DetailRow
                      label='Legal Status'
                      value={selectedAccount.legalStatus}
                    />
                    <DetailRow
            ...
            <DialogFooter className='mt-6 flex flex-col gap-2 border-t pt-4 sm:flex-row sm:justify-between'>
              ...
              >
                {isMarkingEligible
                  ? 'Marking…'
                  : 'Mark as Recalled Asset Eligible'}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
