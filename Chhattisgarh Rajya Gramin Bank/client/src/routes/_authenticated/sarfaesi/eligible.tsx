...
function RouteComponent() {
  ...
  const canInitiate = useCanAccess('sarfaesi', 'initiate')
  ...
  const handleDownloadProposalCumControl = async (accountNumber: string) => {
    if (!canDownloadDocument) {
      toast.error('You do not have permission to download SARFAESI documents.')
      return
    }
  ...
  const initiateSarfaesi = (accountNumber: string) => {
    if (!canInitiate) {
      toast.error('You do not have permission to initiate SARFAESI.')
      return
    }
              ...
              <PaginatedTable<SarfaesiEligibleAccountRow>
                ...
                renderActions={(row) => {
                  ...
                  const proposalPdfButton =
                    canDownloadDocument && approvalDone && performaCompleted ? (
                      <Button
                        type='button'
                        variant='outline'
                        size='sm'
