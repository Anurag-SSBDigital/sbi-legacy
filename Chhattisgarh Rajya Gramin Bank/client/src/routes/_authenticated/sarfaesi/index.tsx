...
export const Route = createFileRoute('/_authenticated/sarfaesi/')({
  component: RouteComponent,
})

function RouteComponent() {
  const canMarkEligible = useCanAccess('sarfaesi', 'mark_eligible')

  const [branchId, setBranchId] = useState<string | undefined>(undefined)
  const [query, setQuery] = useState('')
  const [detailsDialogOpen, setDetailsDialogOpen] = useState(false)
  const [selectedAccount, setSelectedAccount] = useState<
    components['schemas']['SarfaesiEligibleAccount'] | null
  >(null)
  ...
  const {
    ...
    error,
    refetch,
  } = $api.useQuery('get', '/sarfaesi/accountlist', {
    params: {
      header: {
  ...
  const handleMarkEligible = () => {
    if (!canMarkEligible) {
      toast.error('You do not have permission to mark SARFAESI eligibility.')
      return
    }
    ...
    markEligible(
      ...
      },
      {
        onSuccess: () => {
          ...
          toast.success('Account marked as SARFAESI eligible')
        ...
        onError: () => {
          setIsMarkingEligible(false)
          toast.error('Failed to mark account as SARFAESI eligible')
        },
      }
    )
  }
      ...
      <Main className='px-4 py-2'>
        ...
        {isLoading ? (
          ...
          <div className='py-10 text-center text-gray-500'>
            No SARFAESI accounts available for this branch.
          </div>
        ) : (
          <Card className='col-span-full shadow-lg'>
            <CardHeader className='flex flex-col gap-4 sm:flex-row sm:items-baseline sm:justify-between'>
              <div>
                <CardTitle className='text-2xl font-bold text-gray-800 dark:text-gray-200'>
                  SARFAESI Accounts ({filteredData.length} accounts)
                </CardTitle>
              ...
              <div className='flex flex-row items-center gap-2'>
                ...
                <Input
                  ...
                  aria-label='Search SARFAESI accounts'
                />
              </div>
            </CardHeader>
            ...
            <CardContent className='px-6'>
              <PaginatedTable
                ...
                data={filteredData}
                columns={columns}
                emptyMessage='No SARFAESI accounts to show.'
                renderActions={(row) =>
                  row.acctNo ? (
                    <Button
                      variant='outline'
          ...
          <DialogContent className='max-h-[85vh] overflow-y-auto sm:max-w-4xl'>
            <DialogHeader className='space-y-1'>
              <DialogTitle className='flex items-center justify-between gap-3 text-lg font-semibold'>
                <span>SARFAESI Account Details</span>

                {selectedAccount && (
                  <span className='inline-flex items-center gap-2 rounded-full border px-3 py-1 text-xs font-medium'>
                    <span className='h-2 w-2 rounded-full bg-amber-500' />
                    IRAC: {selectedAccount.newIrac ?? '-'}
                  </span>
                )}
              </DialogTitle>
                  ...
                  <h3 className='text-muted-foreground flex items-center justify-between text-xs font-semibold tracking-wide uppercase'>
                    ...
                    <span
                      className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[11px] font-medium ${
                        selectedAccount.sarfaesiEligible
                          ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-200'
                          : 'bg-amber-50 text-amber-700 dark:bg-amber-900/30 dark:text-amber-200'
                      }`}
                    >
                      {selectedAccount.sarfaesiEligible
                        ? 'SARFAESI Eligible Marked'
                        : 'SARFAESI Eligible Not Marked'}
                    </span>
                  </h3>
            ...
            <DialogFooter className='mt-6 flex flex-col gap-2 border-t pt-4 sm:flex-row sm:justify-between'>
              ...
              >
                {isMarkingEligible ? 'Marking…' : 'Mark as SARFAESI Eligible'}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
