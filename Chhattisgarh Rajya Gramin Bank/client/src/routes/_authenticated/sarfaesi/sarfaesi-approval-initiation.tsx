...
interface AccountSummaryCustomer {
  custName?: string
  address?: string
  branchCode?: string
  branchName?: string
  outstand?: number | string | null
  add1?: string
  add2?: string
  add3?: string
  add4?: string
...
function RouteComponent() {
  ...
  const customerDetails = customer?.customer as
  ...
  const onSubmit = (values: FormValues) => {
    if (!canInitiate) {
      toast.error('You do not have permission to initiate SARFAESI approvals.')
      return
    }
