...
export const Route = createFileRoute('/_authenticated/valuer/')({
 ...
 * still $api.useMutation, but avoids OpenAPI type explosions for FormData/blob endpoints.
 */
...
function RouteComponent() {
  ...
  const {
    ...
    isError,
    refetch: refreshData,
  } = $api.useQuery('get', '/valuer/events/by-username', {
    params: {
      header: {
  ...
  /* =========================
     ...
     POST /valuer/events/upload-documents
     ========================= */
  const uploadDocsMutation = useMutationShallow(
    'post',
    '/valuer/events/upload-documents',
    {
      onSuccess: () => {
  ...
  /* =========================
     ...
     GET /valuer/events/{eventId}/download-documents
     ========================= */
  ...
  const handleDownload = async (eventId: number) => {
    try {
      const res = await downloadZipMutation.mutateAsync({
        params: {
          path: { eventId }, // must be number
          header: {
            Authorization: `Bearer ${token}`,
            Accept: '*/*',
          },
        },
        // some clients support it; harmless if ignored
        parseAs: 'blob',
      } as unknown)
        ...
        render: (_, row) => (
          <div className='flex gap-2'>
            ...
            {row.assignment?.status === 'COMPLETED' && (
              ...
              >
                {downloadZipMutation.isPending ? 'Downloading…' : 'Download Document'}
              </Button>
            )}
          </div>
        ),
      ...
      <Main className='px-4 py-2'>
        ...
        {isLoading ? (
          ...
          <div className='mb-6 rounded-lg bg-red-100 p-4 text-red-800'>
            Failed to load valuer events. Please try again later.
          </div>
        ) : (
          <PaginatedTable
            data={transformedData}
            columns={columns}
            emptyMessage='No valuer events assigned to you.'
          />
        )}
              ...
              <div className='space-y-8 text-sm'>
                ...
                {selectedEvent.assignment && (
                  <section>
                    ...
                    <div className='grid grid-cols-2 gap-x-6 gap-y-4'>
                      <div>
                        <strong>Due Date:</strong> {selectedEvent.assignment.dueDate}
                      </div>
                      <div>
                        <strong>Assigned By:</strong> {selectedEvent.assignment.assignedBy}
                      </div>
                      <div>
                        <strong>Assigned At:</strong>{' '}
                        {new Date(selectedEvent.assignment.assignedAt ?? '').toLocaleString()}
                      </div>
                    </div>
                  </section>
                )}
                ...
                {selectedEvent.borrowerDetails && (
                  <section>
                    ...
                    <div className='grid grid-cols-2 gap-x-6 gap-y-4'>
                      <div>
                        <strong>Name:</strong> {selectedEvent.borrowerDetails.custName}
                      </div>
                      <div>
                        <strong>Phone:</strong> {selectedEvent.borrowerDetails.telNo}
                      </div>
                ...
                {selectedEvent.branchDetails && (
                  <section>
                    ...
                    <div className='grid grid-cols-2 gap-x-6 gap-y-4'>
                      <div>
                        <strong>Branch Name:</strong> {selectedEvent.branchDetails.branchName}
                      </div>
                      <div>
                        <strong>Branch Code:</strong> {selectedEvent.branchDetails.branchCode}
                      </div>
                      <div>
                        <strong>Department:</strong> {selectedEvent.branchDetails.departmentName}
                      </div>
                      <div>
                        <strong>District:</strong> {selectedEvent.branchDetails.district}
                      </div>
                      <div>
                        <strong>State:</strong> {selectedEvent.branchDetails.state}
                      </div>
                      <div>
                        <strong>City:</strong> {selectedEvent.branchDetails.city}
                      </div>
                    </div>
                  </section>
                )}
        ...
        <Dialog open={submitDialogOpen} onOpenChange={setSubmitDialogOpen}>
          <DialogContent className='max-w-lg rounded-2xl p-8 shadow-xl'>
            ...
            <form onSubmit={handleSubmit} className='mt-4 space-y-8'>
              <div>
                <Label htmlFor='doc' className='mb-2 block text-sm leading-6 font-medium'>
                  Upload Documents
                </Label>
              ...
              <DialogFooter className='pt-2'>
                <Button type='submit' className='w-full' disabled={uploadDocsMutation.isPending}>
...
//         `${import.meta.env.VITE_APP_API_URL}/valuer/events/upload-documents`,
//         {
//           method: 'POST',
//           headers: {
//             Authorization: `Bearer ${token}`,
...
//     try {
//       const response = await fetch(
//         `${import.meta.env.VITE_APP_API_URL}/valuer/events/${eventId}/download-documents`,
//         {
//           method: 'GET',
//           headers: {
//             Authorization: `Bearer ${token}`,
