...
export default function BranchAlertsView({
  ...
  const downloadReport = useCallback(async () => {
    ...
    try {
      ...
      if (!includeAll) {
        selectedStages.forEach((status) => params.append('status', status))
      }
      if (selectedDesc && selectedDesc !== 'All') {
        params.set('masterDescription', selectedDesc)
      }
      ...
      const res = await fetch(
        `${BASE_URL}/alert/resolutions/${alertType}/branch/getAlerts/report?${params.toString()}`,
        {
      ...
      link.href = url
      link.download = `alerts_report_${alertType}_${branch ?? 'branch'}_${selectedStages.join('_') || 'all'}.pdf`
  ...
  const tabs = useMemo(() => {
    type AlertSummaryDTO = components['schemas']['AlertSummaryDTO']
