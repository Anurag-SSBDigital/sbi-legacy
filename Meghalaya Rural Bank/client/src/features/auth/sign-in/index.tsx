...
//         <div className='relative mt-8 h-25 w-90'>
...
export default function SignInPage() {
  ...
  return (
    <div className='relative flex min-h-screen w-full flex-col items-center justify-center overflow-hidden bg-gradient-to-br from-[#f7fafc] via-white to-[#eef6ff] dark:from-[#0b1220] dark:via-[#0f172a] dark:to-[#0b1220]'>
      {/* Hand-drawn doodle layer (adaptive to theme) */}
      <svg
        aria-hidden
        className='pointer-events-none absolute inset-0 h-full w-full opacity-40 dark:opacity-25'
        viewBox='0 0 1440 900'
      ...
      >
        <g
          ...
          strokeLinecap='round'
          strokeLinejoin='round'
          className='text-slate-800/70 dark:text-white/50'
        >
          <path
            d='M40 70 C 120 30, 200 120, 280 80'
            strokeDasharray='420'
      ...
      {/* Header */}
      <div className='z-10 mb-8 flex flex-col items-center px-4 text-center'>
        <div className='relative mt-2 h-16 w-auto'>
          <img
            src={ViteLogo}
            alt='SSBD Logo'
      ...
      <div className='z-10 w-[92%] max-w-md rounded-3xl border border-slate-200/70 bg-white/90 p-6 shadow-[0_20px_60px_-20px_rgba(0,0,0,0.35)] backdrop-blur-md transition-transform duration-300 hover:scale-[1.01] dark:border-slate-700/60 dark:bg-slate-900/85'>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-6'>
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      placeholder='name@example.com'
                      {...field}
                      className='h-11 rounded-xl border-slate-300/70 bg-white/80 text-slate-900 placeholder:text-slate-400 focus-visible:ring-[color:var(--ring)] dark:border-slate-700 dark:bg-slate-800/90 dark:text-white'
                      autoComplete='username'
                      aria-label='Username'
                    />
                  </FormControl>
            ...
            <Button
              type='submit'
              className='relative mt-2 h-11 w-full rounded-xl bg-[color:var(--primary)] text-[15px] font-bold text-white shadow-lg focus-visible:ring-2 focus-visible:ring-[color:var(--ring)] focus-visible:ring-offset-2 focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-70'
              disabled={isLoading}
            >
              <span className='relative z-10'>
                {isLoading ? 'Logging in…' : 'Login'}
