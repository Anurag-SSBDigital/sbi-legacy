  ...
  const downloadReport = useCallback(async () => {
    ...
    try {
      ...
      if (selectedMasterId !== undefined) {
        params.set('masterDescriptionId', String(selectedMasterId))
      }
      if (selectedSeverity) params.set('severityLevel', selectedSeverity)
      if (selectedAlertDate) params.set('alertDate', selectedAlertDate)
      if (fromDate) params.set('fromDate', fromDate)
      if (toDate) params.set('toDate', toDate)
      params.set('size', '100000')
      stagesForRequest.forEach((stage) => params.append('stages', stage))

      const token = sessionStorage.getItem('token')
      const res = await fetch(
        `${BASE_URL}/alert/resolutions/getAll/report?${params.toString()}`,
        {
      ...
      link.href = url
      link.download = `alerts_report_${alertType}_${selectedStages.join('_') || 'all'}.pdf`
