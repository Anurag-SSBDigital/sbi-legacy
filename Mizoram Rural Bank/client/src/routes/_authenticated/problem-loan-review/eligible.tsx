...
//   '/_authenticated/problem-loan-review/eligible'
// )({
...
//     error,
//     refetch: refetchList,
//   } = $api.useQuery('get', '/problem-loan-reviews/eligible-accounts')

//   const initiateProblemLoanReviewMutation = $api.useMutation(
//     'post',
//     '/api/problemloanreview/{accountNumber}/initiate'
//   )

//   const initiateMutation = (accountNumber: string) =>
//     initiateProblemLoanReviewMutation.mutate(
//       {
...
//     {
//       params: {
//         path: { acctNo: selectedAccountNumber ?? '' },
//       },
//     },
...
//     'get',
//     '/api/problemloanreview/{accountNumber}/status',
//     {
//       params: {
...
//   const loadingDetails = accountDataLoading || statusLoading
...
//     const values =
//       accounts
//         ?.map((d) => Number(d.outstandingAmount))
...
//       key: 'address',
//       label: 'City',
//       render: (_value, row) => <span>{row.address ? row.address : '-'}</span>,
//     },
//     // {
//     //   key: 'newIrac',
//     //   label: 'IRAC Code',
...
//           currentPage={{
//             type: 'label',
//             label: 'Problem Loan Review Eligible Accounts',
//           }}
//         />
...
//             Error fetching eligible Problem Loan Review accounts:{' '}
//             {String((error as Error).message)}
//           </div>
//         ) : !accounts || accounts.length === 0 ? (
//           <div className='py-10 text-center text-gray-500'>
//             No Problem Loan Review eligible accounts available.
//           </div>
//         ) : (
...
//                   Problem Loan Review Eligible Accounts ({filteredData.length}{' '}
//                   accounts)
//                 </CardTitle>
...
//                   Search eligible Problem Loan Review accounts
//                 </Label>
//                 <Input
//                   id='searchEligible'
...
//                   aria-label='Search eligible Problem Loan Review accounts'
//                 />
//               </div>
//             </CardHeader>
...
//                 data={filteredData}
//                 columns={columns}
//                 emptyMessage='No Problem Loan Review eligible accounts to show.'
//                 renderActions={(row) => {
...
//                   const completed =
//                     row.reviewCreated === true && row.workflowInitiated === true
//                   return (
...
//                       {!row.reviewCreated && (
//                         <Link
//                           to='/problem-loan-review/review/$accountNumber'
//                           params={{ accountNumber: row.accountNumber }}
//                         >
//                           <Button
//                             type='button'
//                             variant='outline'
...
//                       {row.reviewCreated && !row.workflowInitiated && (
//                         <Button
//                           type='button'
//                           variant='outline'
//                           onClick={() =>
//                             handleInitiateClick(String(row.accountNumber))
//                           }
//                           className={baseButtonClass}
//                         >
//                           Initiate Problem Loan Review
//                         </Button>
//                       )}
...
//                         className='text-xs'
//                         onClick={() =>
//                           handleOpenDetails(String(row.accountNumber as string))
//                         }
//                       >
//                         View
//                       </Button>
...
//       {/* Confirm Initiate Problem Loan Review */}
//       <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
//         <AlertDialogContent>
//           <AlertDialogHeader>
//             <AlertDialogTitle>Are you sure?</AlertDialogTitle>
//             <AlertDialogDescription>
//               This action will initiate the Problem Loan Review process for
...
//         }}
//       >
//         <DialogContent className='max-h-[90vh] max-w-4xl sm:max-w-5xl'>
//           <DialogHeader>
...
//             </DialogTitle>
//             <DialogDescription>
//               Combined view of core account information and Problem Loan Review
//               process status for quick review.
//             </DialogDescription>
//           </DialogHeader>
...
//               <div className='grid gap-4 md:grid-cols-3'>
//                 {/* Account overview */}
//                 <div className='bg-muted/40 space-y-3 rounded-lg border p-4 md:col-span-2'>
...
//                         <p className='font-medium'>
//                           IRAC: {accountDetails.iracStatus ?? '-'}
//                         </p>
//                         <p className='text-muted-foreground text-xs'>
//                           NPA Date: {accountDetails.npaDate ?? '-'}
//                         </p>
//                       </div>
//                       <div className='space-y-1'>
//                         <p className='text-muted-foreground'>Sanction & Due</p>
//                         <p className='text-xs'>
//                           Sanction: {accountDetails.loanSanctionDate ?? '-'}
//                         </p>
//                         <p className='text-xs'>
//                           Reason for NPA: {accountDetails.reasonForNPA ?? '-'}
//                         </p>
//                         <p className='text-xs'>
//                           Next Review Date:{' '}
//                           {accountDetails.nextReviewDate ?? '-'}
//                         </p>
//                       </div>
//                       <div className='space-y-1'>
...
//                       <Badge
//                         className={processStatusBadgeClass(statusData.overdue)}
//                       >
...
//                       {typeof statusData.progressPercentage === 'number' && (
//                         <div className='space-y-1'>
...
//                               style={{
//                                 width: `${Math.min(
//                                   Math.max(statusData.progressPercentage, 0),
//                                   100
//                                 )}%`,
//                               }}
//                             />
...
//                             Stage start date
//                           </p>
//                           <p>{statusData.stageStartDate ?? '-'}</p>
//                         </div>
//                         <div>
//                           <p className='text-muted-foreground'>
//                             Stage due date
//                           </p>
//                           <p>{statusData.dueDate ?? '-'}</p>
//                         </div>
//                         <div>
//                           <p className='text-muted-foreground'>Days in stage</p>
//                           <p>{statusData.daysInCurrentStage ?? '-'}</p>
//                         </div>
//                         <div>
//                           <p className='text-muted-foreground'>Branch</p>
//                           <p>{statusData.branchCode ?? '-'}</p>
//                         </div>
//                       </div>

//                       {Array.isArray(statusData.pendingActions) &&
//                         statusData.pendingActions.length > 0 && (
//                           <div className='space-y-1 pt-1'>
//                             <p className='text-[0.7rem] font-medium'>
//                               Pending actions
//                             </p>
//                             <ul className='list-disc space-y-0.5 pl-4 text-[0.7rem]'>
//                               {statusData.pendingActions.map((a: string) => (
//                                 <li key={a}>{a}</li>
//                               ))}
//                             </ul>
//                           </div>
...
//                   ) : (
//                     <p className='text-muted-foreground text-xs'>
//                       Problem Loan Review process not yet initiated for this
//                       account.
//                     </p>
//                   )}
//                 </div>
...
import { ProfileDropdown } from '@/components/profile-dropdown'
import { Search } from '@/components/search'
import { AccountNoCell, CurrencyCell, TooltipCell } from '@/components/table/cells'
import { ThemeSwitch } from '@/components/theme-switch'

export const Route = createFileRoute('/_authenticated/problem-loan-review/eligible')({
  component: RouteComponent,
})
...
/* -----------------------------
   Problem Loan Review Stage-wise summary response (same pattern as OTS)
------------------------------ */
type PlrDocument = {
...
type PlrStageWiseSummaryResponse = {
...
const deriveStageStatus = (stage: PlrStage): 'COMPLETED' | 'IN_PROGRESS' | 'PENDING' => {
...
function RouteComponent() {
  ...
  const initiateProblemLoanReviewMutation = $api.useMutation('post', '/api/problemloanreview/{accountNumber}/initiate')
  ...
  const { data: stageWiseSummary, isLoading: stageWiseLoading } = $api.useQuery(
    'get',
    '/api/problemloanreview/{accountNumber}/stage-wise-summary',
    {
      params: {
        path: { accountNumber: selectedAccountNumber ?? '' },
      },
  ...
  const stageSummary: PlrStageWiseSummaryResponse | null = useMemo(() => {
    if (!stageWiseSummary) return null
    if (hasData<PlrStageWiseSummaryResponse>(stageWiseSummary)) return stageWiseSummary.data
    return stageWiseSummary as unknown as PlrStageWiseSummaryResponse
  }, [stageWiseSummary])

  const stages: PlrStage[] = stageSummary?.stages ?? []
  const history: PlrHistoryItem[] = stageSummary?.history ?? []
  ...
  return (
    <>
      ...
      <Main className='px-4 py-2'>
        <AppBreadcrumb className='p-2' crumbs={[Home]} currentPage={{ type: 'label', label: 'Problem Loan Review Eligible Accounts' }} />
        ...
        {isLoading ? (
          ...
          <div className='py-10 text-center text-red-500'>
            Error fetching eligible Problem Loan Review accounts: {String((error as Error).message)}
          </div>
        ) : !accounts || accounts.length === 0 ? (
          <div className='py-10 text-center text-gray-500'>No Problem Loan Review eligible accounts available.</div>
        ) : (
          <Card className='col-span-full shadow-lg'>
            <CardHeader className='flex flex-col gap-4 sm:flex-row sm:items-baseline sm:justify-between'>
              <div>
                <CardTitle className='text-2xl font-bold text-gray-800 dark:text-gray-200'>
                  Problem Loan Review Eligible Accounts ({filteredData.length} accounts)
                </CardTitle>
                <p className='mt-1 text-xs text-gray-500 dark:text-gray-400'>
            ...
            <CardContent className='px-6'>
              <PaginatedTable
                ...
                renderActions={(row) => {
                  ...
                  const completed = row.reviewCreated === true && row.workflowInitiated === true
                  ...
                  return (
                    <div className='flex flex-row justify-end gap-1'>
                      ...
                      {!row.reviewCreated && (
                        <Link to='/problem-loan-review/review/$accountNumber' params={{ accountNumber: row.accountNumber }}>
      ...
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Are you sure?</AlertDialogTitle>
            <AlertDialogDescription>
              This action will initiate the Problem Loan Review process for account{' '}
                ...
                <div className='grid gap-4 md:grid-cols-3'>
                  ...
                  <div className='rounded-xl border bg-gradient-to-b from-muted/40 to-background p-4 md:col-span-2 shadow-sm'>
                    ...
                    {accountDetails ? (
                      <div className='grid gap-3 text-xs sm:grid-cols-2'>
                        ...
                        <div className='rounded-lg border bg-background/60 p-3'>
                          <p className='text-muted-foreground'>Sanction & Review</p>
                          <p className='mt-1 text-xs'>Sanction: {accountDetails.loanSanctionDate ?? '-'}</p>
                          <p className='mt-1 text-xs'>Reason for NPA: {accountDetails.reasonForNPA ?? '-'}</p>
                          <p className='mt-1 text-xs'>Next Review Date: {accountDetails.nextReviewDate ?? '-'}</p>
                        </div>

                        <div className='rounded-lg border bg-background/60 p-3'>
                          <p className='text-muted-foreground'>Address</p>
                          <p className='mt-1 text-xs'>{accountDetails.address ?? '-'}</p>
                        </div>
                      </div>
                    ) : (
                  ...
                  <div className='rounded-xl border bg-gradient-to-b from-muted/40 to-background p-4 shadow-sm'>
                    <div className='mb-3 flex items-center justify-between gap-2'>
                      <h3 className='text-sm font-semibold tracking-tight'>Problem Loan Review status</h3>
                      {typeof statusData?.overdue === 'boolean' && (
