...
function RouteComponent(): JSX.Element {
  ...
  const accountDetailUrl = useMemo(() => {
    ...
    return `${baseUrl}/account/getAccountDetail?acctNm=${enc}`
  }, [accountId])
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        ...
        const res = await fetch(accountDetailUrl, {
          method: 'GET',
          headers: {
        ...
        if (!res.ok)
          throw new Error(`Account detail fetch failed: ${res.status}`)
        ...
        form.reset(
          {
            ...
            borrowerProfileNote:
              [
                ...
                c.sanctDt
                  ? `Sanction Date: ${formatDateForInput(c.sanctDt)}`
                  : '',
