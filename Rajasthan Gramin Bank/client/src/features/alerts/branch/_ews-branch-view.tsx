...
import { AccountNoCell, DateTimeCell } from '@/components/table/cells.ts'
import BranchSelector from '@/features/dashboard/components/branch-selector.tsx'

export default function EWSBranchView() {
  ...
  return (
    <MainWrapper extra={<BranchSelector value={branch} setValue={setBranch} />}>
      <div className='mx-2 text-2xl font-semibold'>EWS Alerts</div>
...
function ResolutionDialog({ row, onSuccess }: ResolutionDialogProps) {
  ...
  const uploadMutation = $api.useMutation(
    'post',
    '/AlertResolution/{description}/{accountNumber}/upload'
  )
  const submitMutation = $api.useMutation('post', '/AlertResolution/create')
