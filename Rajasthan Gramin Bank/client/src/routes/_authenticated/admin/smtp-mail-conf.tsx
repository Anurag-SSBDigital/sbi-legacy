...
function extractList(payload: unknown): MailConfig[] {
  ...
  if (isRecord(payload)) {
    ...
    if (isRecord(d) && Array.isArray(d.content)) return d.content as MailConfig[]
  }
  return []
}
...
function RouteComponent() {
  ...
  /* =========================
     ...
     ========================= */
  const deleteMutation = useMutationShallow('delete', '/mail/configuration/{id}')
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (isRecord(res) && typeof res.status === 'string' && res.status !== 'SUCCESS') {
        throw new Error(typeof res.message === 'string' ? res.message : 'Failed to delete')
      }
  ...
  /* =========================
     ...
     ========================= */
  const setActiveMutation = useMutationShallow('patch', '/mail/update-status/{id}')
  ...
  const handleSetActive = async (cfg: MailConfig) => {
    ...
    try {
      ...
      if (isRecord(res) && typeof res.status === 'string' && res.status !== 'SUCCESS') {
        throw new Error(typeof res.message === 'string' ? res.message : 'Failed to update status')
      }
  ...
  const busy =
    !!configsLoading || !!deleteMutation.isPending || !!setActiveMutation.isPending
          ...
          {configList.length === 0 ? (
            ...
            <div className='mt-4 grid gap-3 md:grid-cols-2 lg:grid-cols-3'>
              {configList.map((cfg) => {
                ...
                return (
                  <button
                    key={cfg.id ?? `${cfg.fromMailId}-${cfg.smtpHost}-${cfg.smtpPort}`}
                    type='button'
            ...
            <div className='flex items-center gap-3'>
              ...
              <div>
                ...
                <p className='text-[11px] text-slate-500 dark:text-slate-400'>
                  {config ? 'Selected profile details.' : 'Select a configuration above.'}
                </p>
              </div>
            </div>
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    try {
      if (values.id) {
        ...
        if (isRecord(res) && typeof res.status === 'string' && res.status !== 'SUCCESS') {
          throw new Error(typeof res.message === 'string' ? res.message : 'Failed to update')
        }

        const saved =
          isRecord(res) && isRecord(res.data) ? (res.data as MailConfig) : (res as MailConfig)
        ...
        if (isRecord(res) && typeof res.status === 'string' && res.status !== 'SUCCESS') {
          throw new Error(typeof res.message === 'string' ? res.message : 'Failed to create')
        }
      ...
      <DialogContent className='max-h-[90vh] overflow-hidden rounded-2xl border border-slate-200/70 bg-background p-0 shadow-2xl sm:max-w-xl dark:border-slate-800 dark:bg-slate-950'>
          ...
          <div className='space-y-3 rounded-xl border border-slate-200/70 bg-slate-50/80 p-4 dark:border-slate-800 dark:bg-slate-900/60'>
            <div className='grid gap-1.5'>
              ...
              <Input
                ...
                {...register('fromMailId', { required: 'From Email is required.' })}
                className={`h-9 text-sm ${errors.fromMailId ? 'border-red-500 focus-visible:ring-red-500/40' : ''
                  }`}
              />
              {errors.fromMailId && (
        ...
        <DialogFooter className='rounded-b-2xl border-t border-slate-200/70 bg-slate-50 px-6 py-4 dark:border-slate-800 dark:bg-slate-900'>
          <div className='flex w-full items-center justify-end gap-2'>
            ...
            <Button type='submit' form='mail-config-form' disabled={busy} className='text-white'>
              {defaultValues?.id ? 'Update Configuration' : 'Save Configuration'}
            </Button>
          </div>
        </DialogFooter>
      </DialogContent>
