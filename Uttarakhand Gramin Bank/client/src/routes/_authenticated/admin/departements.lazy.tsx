...
function exportCSV<T extends Record<string, unknown>>(
  ...
  const lines = [headerLabels.map(escape).join(','), ...matrix.map((r) => r.map(escape).join(','))]
...
async function readExcelRows(file: File): Promise<UploadRow[]> {
  ...
  const rows: UploadRow[] = json.map((r) => ({
    departmentName: String(pickVal(r, ['departmentName', 'Department Name', 'Name']) ?? '').trim(),
    departmentCode: String(pickVal(r, ['departmentCode', 'Department Code', 'Code']) ?? '').trim(),
    ...
    parentDepartmentId: String(pickVal(r, ['parentDepartmentId', 'Parent Department Id']) ?? '').trim() || undefined,
    parentDepartmentName: String(pickVal(r, ['parentDepartmentName', 'Parent Department Name']) ?? '').trim() || undefined,
  }))

  // remove fully empty rows
  return rows.filter((x) => Object.values(x).some((v) => String(v ?? '').trim() !== ''))
}

function validateUploadRows(rows: UploadRow[]): { ok: UploadRow[]; errors: UploadError[] } {
    ...
    <div className='space-y-2'>
      <div className='flex flex-wrap items-center justify-between gap-2 text-xs text-[var(--color-muted-foreground)]'>
        <div className='flex items-center gap-2'>
          ...
          {running && <span className='rounded-full border px-2 py-0.5'>⏳ Running</span>}
        </div>
      ...
      {currentLabel && (
        <div className='text-xs text-[var(--color-muted-foreground)]'>
          Now: <span className='font-medium text-[var(--color-foreground)]'>{currentLabel}</span>
        </div>
      )}
    </div>
  )
...
function BulkUploadDepartments({
  onDone,
}: {
  onDone: () => void
}) {
  ...
  const parentNameToId = useMemo(() => {
    ...
    parentDepartmentsResponse?.data?.forEach((p) => m.set(p.name.trim().toLowerCase(), p.id))
    return m
  }, [parentDepartmentsResponse])

  const createDepartmentMutation = $api.useMutation('post', '/departments/create')
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) clearAll() }}>
        ...
        <DialogContent className='max-h-[90vh] overflow-y-auto p-0 sm:max-w-2xl'>
          ...
          <div className='space-y-4 p-6 pt-4'>
            ...
            <div className='flex flex-wrap items-center gap-2'>
              <Button
                ...
                onClick={async () => {
                  try {
                    ...
                    toast.error(`Failed to generate demo excel: ${neverToError(e instanceof Error ? e : new Error('Unknown error'))}`)
                  }
                }}
              >
                Download Demo Excel
              ...
              <Button type='button' size='sm' onClick={pickFile} disabled={running} className='text-white'>
                Choose File
              </Button>
            ...
            <div className='rounded-xl border bg-[var(--color-card)] p-4 shadow-sm'>
              <div className='text-sm font-semibold'>Run Errors</div>
              {runErrors.length === 0 ? (
                <div className='mt-1 text-sm text-[var(--color-muted-foreground)]'>No runtime errors yet.</div>
              ) : (
...
function RouteComponent() {
  ...
  const [selectedParentDepartmentNames, setSelectedParentDepartmentNames] = useState<string[]>([])
  const [selectedCities, setSelectedCities] = useState<string[]>([])

  const canCreate = useCanAccess('departments', 'create')
  const [canUpdate, canDelete] = usePermissions('departments', ['update', 'delete'])

  const { data: departmentsData, isLoading, refetch } = $api.useQuery('get', '/departments/list', {
    params: {
      header: { Authorization: `Bearer ${sessionStorage.getItem(`token`) || ``}` },
    },
  })

  const deleteDepartmentMutation = $api.useMutation('delete', '/departments/delete/{id}')
  ...
  type DepartmentRow = (typeof departments)[number] & { parentDepartmentName: string }
  ...
  const distinctParentDepartmentNames = useMemo(
    () => [...new Set(transformedDepartments.map((dept) => dept.parentDepartmentName))].sort(),
  ...
  const filteredDepartments: DepartmentRow[] = useMemo(
    () =>
      transformedDepartments.filter((dept) => {
        ...
        const cityMatch = selectedCities.length === 0 || selectedCities.includes(dept.city || '')
        return parentMatch && cityMatch
      }),
    [transformedDepartments, selectedParentDepartmentNames, selectedCities]
  )

  const isFiltered = selectedParentDepartmentNames.length > 0 || selectedCities.length > 0
        ...
        {selectedParentDepartmentNames.map((name) => (
          <Badge key={name} variant='secondary'>
            ...
            <Button
              ...
              size='sm'
              className='ml-1'
              onClick={() => setSelectedParentDepartmentNames((prev) => prev.filter((n) => n !== name))}
            >
              ×
            </Button>
          </Badge>
        ...
        !isLoading && (
          <div className='flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-gray-300 p-12 text-center dark:border-gray-700'>
            <h3 className='text-xl font-semibold text-gray-700 dark:text-gray-300'>
              {isFiltered ? 'No departments match the selected filters.' : 'No Departments Found'}
            </h3>
            {!isFiltered && (
      ...
      {viewDepartment && (
        <Dialog open={viewDialogOpen} onOpenChange={setViewDialogOpen}>
          <DialogContent className='max-w-xl'>
            <DialogHeader>
              <DialogTitle className='text-xl font-bold'>Department Details</DialogTitle>
              <DialogDescription>Full information about the department</DialogDescription>
            </DialogHeader>
            ...
            <div className='grid grid-cols-2 gap-4 text-sm text-gray-800 dark:text-gray-100'>
              ...
              <div>
                <p className='text-muted-foreground mt-3 font-semibold'>State</p>
                <p>{viewDepartment.state}</p>
              </div>
              <div>
                <p className='text-muted-foreground font-semibold'>Parent Department</p>
...
function CreateDepartmentForm({
  ...
  const { data: parentDepartmentsResponse, isLoading: parentDepartmentsLoading } =
    $api.useQuery('get', '/departments/get/dropdown', {
      params: {
        header: {
  ...
  const createDepartmentMutation = $api.useMutation('post', '/departments/create')
  const updateDepartmentMutation = $api.useMutation('put', '/departments/update/{id}')
  ...
  const onSubmit = handleSubmit((values) => {
    ...
    if (values.id) {
      updateDepartmentMutation.mutate(
        {
          body: payload,
          params: { path: { id: values.id }, header: { Authorization: `Bearer ${token}` } },
        },
        {
          onSuccess: () => {
      ...
      createDepartmentMutation.mutate(
        { body: payload, params: { header: { Authorization: `Bearer ${token}` } } },
        {
          onSuccess: (res) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className='max-h-[90vh] overflow-y-auto p-0 sm:max-w-xl'>
        ...
        <form id='create-dept-form' onSubmit={onSubmit} className='space-y-5 px-6 pt-2 pb-2'>
          {inputFields.map(([name, label, placeholder]) => (
            <div key={name} className='grid gap-1.5'>
              ...
              <Input
                ...
                {...register(name, { required: `${label.replace(` *`, ``)} is required.` })}
        ...
        <DialogFooter className='rounded-b-lg bg-gray-50 p-6 pt-4 dark:bg-gray-800'>
          ...
          <Button type='submit' form='create-dept-form' disabled={isSubmitting} className='text-white'>
